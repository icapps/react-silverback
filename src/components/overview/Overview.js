      ...
      return {
        ...
        handleAction: async item => {
          ...
          if (actionResult.action && actionResult.action.type.includes('FULFILLED')) {
            this.props.setMessage({
              type: identifiers.MESSAGE_SUCCESS,
              text: strings.formatString(action.successMessage, {
                item: <b>{item.name}</b>,
              }),
            });
          }
      ...
      <main className="overview col-sm-9 offset-sm-3 col-md-10 offset-md-2 pt-3">
        <div className="container">
          <h2>
            {props.title}
            {props.sortField && (
              <span className="sort-label">{`${strings.SORTED_BY} ${getSortLabel(props.sortField)} (${
                props.sortOrder === SORT_DESC ? strings.DESCENDING : strings.ASCENDING
              })`}</span>
            )}
          </h2>
          <div className="overview-settings">
            <Filter handleFilter={this.handleFilter} />
            {props.create && this.props.createParameters.length && (
              <CreateModal
          ...
          {props.listItems.length > 0 ? (
            <React.Fragment>
              <Pagination
                totalCount={props.paginationTotalCount}
            ...
            <div className="jumbotron" role="alert">
              <span className="empty-overview">
                {strings.formatString(strings.NO_RESULTS_FOUND, {
                  result: props.title,
                })}
              </span>
            </div>
