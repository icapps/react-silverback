        ...
        if (index + 1 >= this.state.minPage && index + 1 <= this.state.maxPage) {
          return (
            <li
              ...
              className={`page-item ${this.props.activePage === index ? 'active' : ''}`}
              onClick={() => this.changePage(index)}
            >
              <span className="page-link">
                {this.props.activePage === index && <span className="sr-only">{strings.CURRENT}</span>}
                {index + 1}
              </span>
            </li>
          );
      ...
      <div className="pagination-container">
        <div className="pagination-group pages">
          <span className="pagination-label">
            {strings.formatString(strings.SHOWING_X_OF_X, {
              startOfRange,
              endOfRange: endOfRange > props.totalCount ? props.totalCount : endOfRange,
          ...
          {nrOfPages !== 1 && (
            <nav aria-label="Pagination">
              <ul className="pagination">
                <li
                  className={`page-item ${props.activePage === 0 ? 'disabled' : ''}`}
                  onClick={() => props.activePage !== 0 && this.changePage(0)}
                >
                ...
                <li
                  className={`page-item ${props.activePage === 0 ? 'disabled' : ''}`}
                  onClick={() => props.activePage !== 0 && this.changePage(props.activePage - 1)}
                >
                ...
                <li
                  className={`page-item ${props.activePage === nrOfPages - 1 ? 'disabled' : ''}`}
                  onClick={() => props.activePage !== nrOfPages - 1 && this.changePage(props.activePage + 1)}
                >
                ...
                <li
                  className={`page-item ${props.activePage === nrOfPages - 1 ? 'disabled' : ''}`}
                  onClick={() => props.activePage !== nrOfPages - 1 && this.changePage(nrOfPages - 1)}
                >
        ...
        <div className="pagination-group">
          <Dropdown
            ...
            handleChange={this.changePageLimit}
