...
class LanguageCodeDetail extends Component {
    ...
    return new Promise(async resolve => {
      ...
      if (result.action && result.action.type === constants.CREATE_LANGUAGE_CODE_FULFILLED) {
        this.props.history.replace(
          `/${window.location.pathname.split('/')[1]}/${this.props.code.id}`,
          this.props.code.id,
        );
  ...
  deprecateLanguageCode = async id => {
    ...
    if (result.action && result.action.type === constants.DEPRECATE_LANGUAGE_CODE_FULFILLED) {
      ...
      this.props.setMessage({
        ...
        text: strings.formatString(strings.DEPRECATED_SUCCESS, { item: <b>{this.props.code.name}</b> }),
      });
    }
  };
  ...
  undeprecateLanguageCode = async id => {
    ...
    if (result.action && result.action.type === constants.UNDEPRECATE_LANGUAGE_CODE_FULFILLED) {
      ...
      this.props.setMessage({
        type: identifiers.MESSAGE_SUCCESS,
        text: strings.formatString(strings.UNDEPRECATED_SUCCESS, { item: <b>{this.props.code.name}</b> }),
      });
    }
  };
          ...
          inputItems={[
            {
              ...
              value: format(new Date(props.code.createdAt), 'DD-MM-YYYY'),
              type: 'plain',
              isEditable: false,
            },
